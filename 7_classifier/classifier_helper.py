...
def _build_pipeline(search_space):
    """Infers the correct pipeline from the search_space keys."""
    keys = list(search_space.keys())
    if any('cleaning' in k or 'balancing' in k for k in keys):
        # Undersampling pipeline: ENN cleaning + RandomUnderSampler balancing
...
def model_development_classifier(data, non_feature_cols, target_col, trials,
    ...
    default_estimator = (_build_pipeline(search_space) if use_pipeline
                         else lgb.LGBMClassifier(random_state=42, n_jobs=-1, verbose=-1))
    ...
    base_estimator = (_build_pipeline(search_space) if use_pipeline
                      else lgb.LGBMClassifier(random_state=42, n_jobs=-1, verbose=-1))
