...
def save_checkpoint(
    ...
    checkpoint_filename = (
        f"checkpoint_epoch_{epoch:03d}_final.pt"
        if is_final else
        f"checkpoint_epoch_{epoch:03d}_{ckpt_tag}.pt"
    )
...
def evaluate_fold(
    ...
    print(f"Fold {fold_idx}: Training RNN on cpu")
...
def set_freeze_mode_rnn(model, freeze_level=0):
    ...
    if freeze_level >= 1:
        print("Freeze Level 1: freezing RNN")
    ...
    if freeze_level >= 2:
        print("Freeze Level 2: freezing RNN + first dense block")
        for i in range(min(4, len(seq))):
            for p in seq[i].parameters():
                p.requires_grad = False

    if freeze_level >= 3:
        print("Freeze Level 3: freezing RNN + first two dense blocks")
        for i in range(4, min(8, len(seq))):
            for p in seq[i].parameters():
                p.requires_grad = False
...
def evaluate_fold_TL(
    ...
    device = torch.device("cpu")
    print(f"Fold {fold_idx}: RNN TL on cpu | freeze={freeze_level} | lr={learning_rate:g}")
    ...
    set_freeze_mode_rnn(model, freeze_level)
