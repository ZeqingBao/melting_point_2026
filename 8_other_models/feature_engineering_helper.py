...
def reduce_features_by_RFE(df_features, df_target, n_features_to_select, tolerance, model, step=1, 
    ...
    if model not in models:
        raise ValueError(f"Unknown model: {model}. Use 'XGBost', 'LightGBM', or 'Random Forest'")
...
def RFE_plot(RFE_results, tolerance, model_type, save_path):
    ...
    ax.set_title(f'RFE Performance vs. Feature Count ({model_type})', fontsize=14, pad=15)
