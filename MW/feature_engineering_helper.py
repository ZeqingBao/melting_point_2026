...
def dataset_featurization(data_with_smiles, selected_features, path, mw_threshold=300):
    df = data_with_smiles.copy()

    # Ensure MW_label exists
    if "MW_label" not in df.columns:
        df["MW_label"] = df["SMILES"].apply(lambda s: check_MW(s, threshold=mw_threshold))

    # Build RDKit features from SMILES only
    smiles = df[['SMILES']]
    rdkit_df = smiles_to_features(smiles, ['rdkit']).drop(columns=['SMILES'], axis=1)

    # Add interactions (only if your selected_features include interaction terms)
    rdkit_int = feature_interaction(rdkit_df)

    # Defensive: remove any stale Ro5 feature names
    selected_features = [f for f in selected_features if "Ro5" not in str(f)]

    # Select the requested features
    selected_df = rdkit_int[selected_features]

    # Final dataset
    final_dataset = pd.concat([df[['SMILES', 'MP', 'Type', 'MW_label']], selected_df], axis=1)
...
def combine_features(df_lowMW_X_final, df_highMW_X_final):
    n_features_low = df_lowMW_X_final.shape[1]
    n_features_high = df_highMW_X_final.shape[1]
    print(f"Number of features in Low MW dataset: {n_features_low}")
    print(f"Number of features in High MW dataset: {n_features_high}")

    overlapping_features = set(df_lowMW_X_final.columns).intersection(set(df_highMW_X_final.columns))
    print(f"Number of overlapping features: {len(overlapping_features)}")

    combined_features = list(set(df_lowMW_X_final.columns).union(set(df_highMW_X_final.columns)))
